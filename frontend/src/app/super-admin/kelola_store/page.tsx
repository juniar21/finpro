...
export default function SuperAdminDashboard() {
  ...
  useEffect(() => {
    ...
    setStores([
      { id: "s1", name: "Toko A", address: "Jl. Merdeka No.1", latitude: -6.200, longitude: 106.816, adminId: "2" },
      { id: "s2", name: "Toko B", address: "Jl. Sudirman No.2", latitude: -6.210, longitude: 106.820 },
    ]);
  }, []);
      ...
      <div className="flex h-screen w-full bg-gray-50">
        ...
        <main className="flex-1 p-8 overflow-auto">
                    ...
                    stores.map((store, i) => (
                      <tr key={store.id} className="border-t hover:bg-gray-50 transition cursor-pointer">
                        ...
                        <td className="p-3 flex gap-3">
                          <button title="Edit Toko" className="text-blue-600 hover:underline focus:outline-none">Edit</button>
                          <button title="Hapus Toko" className="text-red-600 hover:underline focus:outline-none">Hapus</button>
                          <button title="Assign Admin" className="text-purple-600 hover:underline focus:outline-none">Assign Admin</button>
                        </td>
                      </tr>
                    ))
                  )}
          ...
          {isAddModalOpen && (
            <div className="fixed inset-0 bg-black bg-opacity-60 backdrop-blur-sm flex items-center justify-center z-50">
              <div className="bg-white rounded-lg p-6 w-full max-w-md shadow-lg relative">
                <button onClick={() => setIsAddModalOpen(false)} className="absolute top-3 right-3 text-gray-500 hover:text-gray-700 transition" aria-label="Tutup modal"><AiOutlineClose size={24} /></button>
                <h2 className="text-xl font-bold mb-5 text-gray-800 flex items-center gap-2"><BsPersonPlusFill />Tambah Store Admin</h2>
                <input type="text" placeholder="Nama lengkap" value={newUser.name} onChange={e => setNewUser({ ...newUser, name: e.target.value })} className="w-full border border-gray-300 p-3 rounded mb-4 focus:outline-none focus:ring-2 focus:ring-purple-500" required />
                <input type="email" placeholder="Email" value={newUser.email} onChange={e => setNewUser({ ...newUser, email: e.target.value })} className="w-full border border-gray-300 p-3 rounded mb-6 focus:outline-none focus:ring-2 focus:ring-purple-500" required />
          ...
          {isStoreModalOpen && (
            <div className="fixed inset-0 bg-black bg-opacity-60 backdrop-blur-sm flex items-center justify-center z-50">
              <div className="bg-white rounded-lg p-6 w-full max-w-md shadow-lg relative">
                <button onClick={() => setIsStoreModalOpen(false)} className="absolute top-3 right-3 text-gray-500 hover:text-gray-700 transition" aria-label="Tutup modal"><AiOutlineClose size={24} /></button>
                <h2 className="text-xl font-bold mb-5 text-gray-800 flex items-center gap-2"><FaStoreAlt />Tambah Toko</h2>
                <input type="text" placeholder="Nama Toko" value={newStore.name} onChange={e => setNewStore({ ...newStore, name: e.target.value })} className="w-full border border-gray-300 p-3 rounded mb-3 focus:outline-none focus:ring-2 focus:ring-green-500" required />
                <input type="text" placeholder="Alamat" value={newStore.address} onChange={e => setNewStore({ ...newStore, address: e.target.value })} className="w-full border border-gray-300 p-3 rounded mb-3 focus:outline-none focus:ring-2 focus:ring-green-500" required />
                <div className="flex gap-4 mb-5">
                  <input type="number" placeholder="Latitude" value={newStore.latitude || ""} onChange={e => setNewStore({ ...newStore, latitude: parseFloat(e.target.value) || 0 })} className="flex-1 border border-gray-300 p-3 rounded focus:outline-none focus:ring-2 focus:ring-green-500" step="any" />
                  <input type="number" placeholder="Longitude" value={newStore.longitude || ""} onChange={e => setNewStore({ ...newStore, longitude: parseFloat(e.target.value) || 0 })} className="flex-1 border border-gray-300 p-3 rounded focus:outline-none focus:ring-2 focus:ring-green-500" step="any" />
                </div>
                <div className="flex justify-end gap-3">
